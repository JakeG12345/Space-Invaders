//
//  MainMenuScene.swift
//  Space Invaders (solo mission)
//
//  Created by Jake Glick on 26/3/21.
//
...
class MainMenuScene: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        let background = SKSpriteNode(imageNamed: "background-imageforspaceinvaders")
        background.position = CGPoint(x: self.size.width/2, y: self.size.height/2)
        background.zPosition = 0
        self.addChild(background)
        
        let gameBy = SKLabelNode(fontNamed: "The Bold Font")
        gameBy.text = "Jake Glick's"
        gameBy.fontSize = 50
